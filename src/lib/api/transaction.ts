...
import type { TransactionType } from '$lib/types/lesson04';

export const token = writable('');
export const loading = writable(true);
export const isAuthenticated = writable(false);

let realToken: string | undefined = undefined;

token.subscribe((value) => {
	realToken = value;
});

export const fetchapi = (path: string, opts = {}) => {
...
const faker_transactions: TransactionType[] = [
	{ id: '1' },
	{ id: '2' },
	{ id: '3' },
	{ id: '4' },
...
export const fetchTransactions = async () => {
	...
	const result = await Promise.resolve({
		data: faker_transactions
	});

	if (!res.ok) {
		throw new Error(`Failed to fetch transactions: ${JSON.stringify(result)}`);
	}
